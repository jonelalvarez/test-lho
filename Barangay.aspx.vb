...
Partial Class Departments
    ...
    Private Sub loadList()
        ...
        With clsMaster
            For Each row In .dynamicQuery("SELECT ID, brgyName, case when status = 1 then 'Active' else 'Inactive' end as status FROM test.brgy_tbl;").AsEnumerable
                htmlStr.Append("<tr>")
                ...
                htmlStr.Append("<td>" + row.Item("brgyName").ToString + "</td>")
    ...
    Private Sub showDetails()
        With clsMaster
            Dim foundRow As DataRow() = .dynamicQuery("SELECT ID, brgyName, case when status = 1 then 'Active' else 'Inactive' end as status FROM test.brgy_tbl; where ID = '" & requestID & "'").Select

            If foundRow.Length > 0 Then
                div_status.Visible = True
                h4_title.InnerText = "View/Edit Record"

                txt_name.Value = foundRow(0).Item("brgyName").ToString
        ...
        If IsPostBack Then
            Try
                Using con As New MySqlConnection(ConnString)
                    ...
                    If _mode = "C" Then
                        sql = "Insert into brgy_tbl (ID, brgyName) values (@ID, @brgyName)"
                    ElseIf _mode = "U" Then
                        sql = "Update brgy_tbl SET ID = @ID, brgyName = @brgyName, status = @status where ID = @ID"
                    End If

                    Using cmd As New MySqlCommand
                        cmd.Parameters.AddWithValue("@ID", requestID)
                        cmd.Parameters.AddWithValue("@brgyName", txt_name.Value.Trim)
    ...
    Public Shared Function validateEntry(mode As String, entry As String) As Boolean
        ...
        With clsMaster2
            If .dynamicQuery("Select ID from brgy_tbl where " + mode + " = '" & entry.Trim & "'").Rows.Count > 0 Then
                Return True
            Else
                Return False
            End If
